...
import com.lenovo.vctl.apps.commons.constants.TimeDateConstant;


public class CalendarUtil {
//	public DateFormat yyyyMMddFormat = new SimpleDateFormat("yyyyMMdd");
	...
	public static int convertTimeMillis2yyyyMMdd(long timeMillis) {
		...
		String str = DateFormatUtils.format(timeMillis, TimeDateConstant.yyyyMMdd);
		return Integer.valueOf(str);
	}

	public static Date convertYyyyMMdd2Date(int yyyyMMdd) {
		try {
			return DateUtils.parseDate(String.valueOf(yyyyMMdd), new String[] { TimeDateConstant.yyyyMMdd });
	 ...
	 * @return
	 */
	public static Map<Long, Integer> formatStringDate(
			Long[] dates) {
	...
	public static Long diffDays(Long after, Long before) {
		...
		return (trimTimeMillis(after) - trimTimeMillis(before)) / DateUtils.MILLIS_PER_DAY;
	}
	...
	public static int getDiffBetweenMonth(Date latestDate, Date current) {
		...
		if (calCurrent.get(Calendar.YEAR) - calLatest.get(Calendar.YEAR) > 0) {
			int diff = calCurrent.get(Calendar.YEAR) - calLatest.get(Calendar.YEAR);
			return (12 - (calLatest.get(Calendar.MONTH) + 1) + 1) + calCurrent.get(Calendar.MONTH) + 12 * (diff - 1);
		} else {
			return calCurrent.get(Calendar.MONTH) - calLatest.get(Calendar.MONTH);
		}
	}
	...
	public static List<Integer> get5ConsensusDays(Long checkPointTime) {
		...
		result.add(CalendarUtil.convertTimeMillis2yyyyMMdd(cal.getTimeInMillis()));
		...
		cal.add(Calendar.DATE, -(TimeDateConstant.DAY_WEEK_AGO_DIFF - TimeDateConstant.PREVIOUS_DAY_DIFF));
		result.add(CalendarUtil.convertTimeMillis2yyyyMMdd(cal.getTimeInMillis()));

		// the day one month before this day
		cal.add(Calendar.DATE, -(TimeDateConstant.DAY_MONTH_AGO_DIFF - TimeDateConstant.DAY_WEEK_AGO_DIFF));
		result.add(CalendarUtil.convertTimeMillis2yyyyMMdd(cal.getTimeInMillis()));

		// the day one season before this day
		cal.add(Calendar.DATE, -(TimeDateConstant.DAY_QUARTER_AGO_DIFF - TimeDateConstant.DAY_MONTH_AGO_DIFF));
		result.add(CalendarUtil.convertTimeMillis2yyyyMMdd(cal.getTimeInMillis()));
	 ...
	 * 
	 * @param time 传入时间
	 * @param yearFormNow 这个值可正可负，某个年份的今日；
	 * @return
	 */
	 ...
	 * @return
	 */
	public static Map<Integer,List<Long>> getSeasonEndTime(int startYear,int endYear,boolean isDesc){
	 ...
	 * @return
	 */
	public static Integer getLastSeasonEndDayYyyyMMdd(Integer currentSeasonEndDay){
