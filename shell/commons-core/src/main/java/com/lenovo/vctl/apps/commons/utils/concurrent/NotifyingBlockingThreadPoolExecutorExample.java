...
public class NotifyingBlockingThreadPoolExecutorExample {	
	...
	private static final Logger LOGGER = Logger.getLogger(NotifyingBlockingThreadPoolExecutorExample.class.getName());
	...
	public static void main(String[] args) {
		...
		//-----------------------------------------------------------------
		int poolSize = 4;
		int queueSize = 8; // recommended - twice the size of the poolSize
...
//		TimeUnit maxBlockingTimeUnit = TimeUnit.MILLISECONDS;
	...
	static private class MyNaiveTask implements Runnable {  
		...
		public void run() {
			...
			int millis = random.nextInt(51 * 1000);
			try {
				Thread.sleep(millis);
